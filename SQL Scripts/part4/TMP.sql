...
ALTER TABLE Jailer_log DROP CONSTRAINT jailer_log_pk;
...
CREATE TABLE Jailer_log (
    jailer_id INT GENERATED BY DEFAULT ON NULL AS IDENTITY,
    jail_address VARCHAR(256) NOT NULL,
    first_name VARCHAR(64) NOT NULL,
    surname VARCHAR(64) NOT NULL,
    birth_date DATE NOT NULL,
    CONSTRAINT jailer_log_pk PRIMARY KEY (jailer_id)
);
...
CREATE OR REPLACE TRIGGER LogTrigger
    BEFORE INSERT ON Jailer
    FOR EACH ROW
BEGIN
        INSERT INTO Jailer_log (jailer_id, jail_address, first_name, surname, birth_date)
        VALUES (:new.jailer_id, (SELECT address FROM Jail WHERE Jail.jail_id = :new.jail_id), :new.first_name, :new.surname, :new.birth_date);
END;
/
/*INSERT INTO Jailer (jailer_id,jail_id, first_name, surname, birth_date)
    VALUES (689, 1, 'Adam','Rabatin',TO_DATE('1999-02-09', 'YYYY-MM-DD'));
SELECT * FROM Jailer_log;*/

--insert test jailer into new created jail
CREATE OR REPLACE TRIGGER TestJailer
    AFTER INSERT ON Jail
    FOR EACH ROW
BEGIN
    INSERT INTO Jailer (jailer_id,jail_id, first_name, surname, birth_date)
    VALUES (999,:new.jail_id,'TestJailer','TestJailer',TO_DATE('1900-01-01', 'YYYY-MM-DD'));
END;
/
INSERT INTO Jail (jail_id, delivery_zone_id, address)
